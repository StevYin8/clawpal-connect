...
export function resolveDefaultRuntimeConfigFilePath() {
    return join(homedir(), ".clawpal-connect", "runtime-config.json");
}
export function createDefaultRuntimeConfig(now = new Date()) {
    return {
        transport: "ws",
...
function normalizeRuntimeConfig(config, fallback) {
    const next = {
        transport: "ws",
