import type { AgentInfo, OpenClawConfig } from "./openclaw_config.js";
...
export async function syncAgentsToRelay(options: AgentSyncOptions): Promise<AgentSyncResult> {
...
export async function deleteAgentFromOpenClaw(
  agentId: string
): Promise<boolean> {
  const config = await readOpenClawConfig();
  if (!config) {
...
export async function getAgentsFromRelay(
  backendUrl: string,
  hostId: string
): Promise<AgentInfo[]> {
  try {
    ...
    const data = await response.json() as { agents?: Array<{ agentId: string; name: string; model: string }> };
